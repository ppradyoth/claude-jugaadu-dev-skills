...
| [`/stash`](skills/stash/) | Mid-change and something urgent cuts in? Shelves your dirty tree onto the stash stack so you go clean, then hands it back with `pop`/`apply` — no `wip` commit. Knows the traps that bite everyone: a plain stash **skips untracked files** (`-u` to include them), `pop` on a conflict does **not** drop the stash, changes come back **unstaged** unless `--index`, and `git stash branch` for when the base moved and pop won't land. | Park it clean, no throwaway commit |
| [`/env-check`](skills/env-check/) | Pulled `main` and the app dies three screens into startup? A new required env var landed and your local `.env` predates it. Diffs the committed `.env.example` — *and* the vars the code actually reads (`process.env.X`, `os.environ[...]`, `env::var(...)`) — against your real `.env`, and reports the gap: missing, undeclared-by-example, and stale. Key names only, values never printed, never writes your `.env`. | The startup crash the repo already knew about |
| [`/revert`](skills/revert/) | A pushed commit broke prod and the instinct is `git reset` + force-push — which rewrites history everyone already pulled. This does it right: `git revert` adds a *new* commit that inverts the bad one, so history stays append-only and the fix pushes as a plain fast-forward. Handles the merge-commit `-m 1` case everyone gets wrong, the "re-merge does nothing, revert the revert" gotcha, and reminds you a revert doesn't un-leak a secret — rotate it. | Undo shared history by moving forward, not erasing |
| [`/cherry-pick`](skills/cherry-pick/) | You need *one* commit on this branch — a hotfix into `release`, a teammate's one-line fix onto `main` — not the whole branch it lives on. `git cherry-pick` replays just that commit's diff here as a **new commit** (new SHA — a copy, not a move). Knows the traps: the exclusive `A..B` range, the empty/already-applied stop, the merge-commit `-m`, `-x` to record the origin, and the big one — cherry-pick now + merge the source branch later lands the change **twice**. | Grab one commit, not the branch |
| [`/rebase`](skills/rebase/) | Your branch is behind `main` and you want the new work *under* yours, linear, no merge bubble. `git rebase` lifts your commits off and replays them on top of the latest base — but replay means **new SHAs**, so it enforces the golden rule (only rebase branches that are yours alone), walks the per-commit conflict loop, and pushes with `--force-with-lease`, never plain `--force`. Knows the reversed `ours`/`theirs` during replay, `rerere`, and when to just *merge* instead. | Replay onto the latest base, safely |
| [`/amend`](skills/amend/) | Committed, then spotted a forgotten file or a typo in the message? Fixes the *last* commit in place with `git commit --amend` instead of stacking a `fix typo` commit on top. Burns in the load-bearing fact — amend **replaces** the commit with a new SHA — so it folds in **staged changes only** (`git diff --staged` first), forces with `--force-with-lease` **only if it was already pushed**, and uses `--reset-author` for a wrong identity. HEAD-only: an older commit is `/squash`, a botched op is `/oops`. | Fix the last commit, no "typo" commit on top |
